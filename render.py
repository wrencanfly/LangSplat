...
def render_set(model_path, source_path, name, iteration, views, gaussians, pipeline, background, args):
    ...
    loss_info_path = os.path.join(render_path, 'loss.info')
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        # FIXME write the loss into the loss file
        # Ll1 = l1_loss(rendering*mask, gt*mask)            
        # loss = Ll1
        # with open(loss_info_path, 'a') as loss_file:
        #     loss_file.write(f"Iteration {idx}, Loss: {loss.item()}\n")
