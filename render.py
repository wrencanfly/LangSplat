...
def render_set(model_path, source_path, name, iteration, views, gaussians, pipeline, background, args):
    ...
    for pos_index in range(1):
        for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
            if idx + 1 in frame_lst:
                output = render(view, gaussians, pipeline, background, pos_index, args)
                if not args.include_feature:
                    rendering = output["render"]
                else:
                    rendering = output["language_feature_image"]
                    # print("rendering shape", rendering.shape) torch.Size([3, 730, 988])
